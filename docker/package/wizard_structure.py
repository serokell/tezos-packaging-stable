...
def replace_systemd_service_env(service_name, field, value):
    for env_file in find_systemd_env_files(show_systemd_service(service_name)):
        ...
        if old is not None:
            new = f"{field}={value}"
            proc_call(f"sudo sed -i 's/{old.group(0)}/{new}/' {env_file}")
...
def get_data_dir(network):
    ...
    if data_dir is None:
        print("TEZOS_NODE_DIR is undefined, defaulting to /var/lib/tezos/node-" + network)
...
class Setup:
    ...
    def fill_baking_config(self):
        ...
        self.config["baker_alias"] = baking_env.get(
            "BAKER_ADDRESS_ALIAS",
            "baker"
        )
