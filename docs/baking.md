...
If at any point after that you want to reset the custom baking service, you can run:

```bash
sudo systemctl start tezos-baking-custom-reset@<chain-name>
```

Alternatively, you can set `RESET_ON_STOP=true` in the `/etc/default/tezos-baking-custom@<chain-name>`
config file and run:
