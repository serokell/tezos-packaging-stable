...
for f in filter(lambda x: x.endswith(".changes"), os.listdir(source_packages_path)):
    subprocess.call(
        f"execute-dput -c dput.cfg {launchpad_ppa} {os.path.realpath(f)}", shell=True
    )
...
for f in filter(lambda x: x.endswith(".src.rpm"), os.listdir(source_packages_path)):
    subprocess.call(
        f"/run/wrappers/bin/sudo -u copr-uploader /run/current-system/sw/bin/copr-cli build {chroots} --nowait {copr_project} {os.path.realpath(f)}",
        shell=True,
    )
